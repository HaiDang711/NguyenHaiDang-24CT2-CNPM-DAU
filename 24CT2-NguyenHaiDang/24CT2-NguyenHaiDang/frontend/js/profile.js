    ...
    try {
      ...
      data.bookings.forEach(booking => {
        ...
        item.innerHTML = `
          ...
          <div class="pay-box" style="margin-top:10px; display:none;"></div>
        ...
        // FR-10: thanh toán — hiện mã QR chuyển khoản thật (VietQR) theo đúng số tiền của đơn.
        // Đây vẫn là MÔ PHỎNG: khách quét QR chuyển khoản xong phải tự bấm "Tôi đã chuyển khoản"
        // để xác nhận, hệ thống KHÔNG tự động dò giao dịch ngân hàng thật.
        if (canPay) {
          const payBox = item.querySelector('.pay-box');
          const btn = document.createElement('button');
          btn.className = 'btn-mini-solid';
          btn.textContent = 'Thanh toán ngay';
          btn.addEventListener('click', () => {
            payBox.style.display = payBox.style.display === 'none' ? 'block' : 'none';
          });
          actionsEl.appendChild(btn);

          const qrInfo = booking.booking_code.replace(/[^a-zA-Z0-9]/g, '').slice(0, 24);
          const qrUrl = `https://img.vietqr.io/image/vpbank-0703562146-compact2.png`
            + `?amount=${Math.round(booking.total_price)}`
            + `&addInfo=${encodeURIComponent(qrInfo)}`
            + `&accountName=${encodeURIComponent('NGUYEN HAI DANG')}`;

          payBox.innerHTML = `
            <div style="border:1px solid var(--line, #E4E0D4); border-radius:10px; padding:16px; text-align:center; max-width:320px;">
              <div style="font-size:13px; color:var(--ink-600, #4A5D5B); margin-bottom:10px;">
                Quét mã bằng app ngân hàng bất kỳ để chuyển khoản đúng số tiền
              </div>
              <img src="${qrUrl}" alt="Mã QR thanh toán" style="width:100%; border-radius:10px;">
              <div style="font-size:12px; color:var(--ink-600, #4A5D5B); margin-top:10px;">
                Nội dung chuyển khoản: <b>${qrInfo}</b>
              </div>
              <button class="btn-mini-solid confirm-paid" style="margin-top:12px; width:100%;">Tôi đã chuyển khoản</button>
              <div style="font-size:11px; color:var(--ink-600, #4A5D5B); margin-top:8px;">
                (Mô phỏng — hệ thống chưa tự động đối soát với ngân hàng thật)
              </div>
            </div>
          `;

          payBox.querySelector('.confirm-paid').addEventListener('click', async () => {
            if (!confirm(`Xác nhận bạn đã chuyển khoản ${moneyVN(booking.total_price)} cho đơn ${booking.booking_code}?`)) return;
            try {
