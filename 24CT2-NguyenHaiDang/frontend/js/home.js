...
async function loadNews(){
  ...
  try {
    ...
    const timeoutId = setTimeout(() => controller.abort(), 8000); // tự bỏ cuộc sau 8 giây nếu server không phản hồi
    ...
    grid.innerHTML = e.name === 'AbortError'
      ? '<p style="color:var(--ink-600)">Máy chủ tin tức phản hồi quá chậm, thử tải lại trang sau nhé.</p>'
      : '<p style="color:var(--ink-600)">Chưa tải được tin tức lúc này, thử lại sau nhé.</p>';
  }
}
